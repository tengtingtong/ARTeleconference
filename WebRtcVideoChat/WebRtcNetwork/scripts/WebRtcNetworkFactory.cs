...
//keep in editor to not miss compiler errors. only add to android builds
#if UNITY_ANDROID || UNITY_EDITOR
        private static bool TryInitAndroid_Old()
        {
            ...
            AndroidJavaClass cPeerConnectionFactory = new AndroidJavaClass("org.webrtc.PeerConnectionFactory");
            //cPeerConnectionFactory.CallStatic("initializeFieldTrials", "");
            AndroidJavaClass cInitializationOptions = new AndroidJavaClass("org.webrtc.PeerConnectionFactory$InitializationOptions");
            AndroidJavaObject oBuilder = cInitializationOptions.CallStatic<AndroidJavaObject>("builder", context);
            AndroidJavaObject oInitializationOptions = oBuilder.Call<AndroidJavaObject>("createInitializationOptions");
            cPeerConnectionFactory.CallStatic("initialize", oInitializationOptions);
        ...
        private static bool TryInitAndroid()
        {
            ...
            bool successful;
            successful = wrtc.CallStatic<bool>("init", context);
            if(successful)
            {
                Debug.Log("android java plugin initialized");
            }
            else
            {
                Debug.LogError("Failed to initialize android java plugin.");
                return false;
            }

            successful = WebRtcCSharp.RTCPeerConnectionFactory.InitAndroidContext(context.GetRawObject());
            ...
            {
                Debug.LogError("Failed to initialize android native plugin.");
            }
            return successful;
        }
#endif
